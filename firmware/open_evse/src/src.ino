...
void TempMonitor::Read()
{
	...
	if ((curms - m_LastUpdate) >= TEMPMONITOR_UPDATE_INTERVAL)
	{
			...
			m_MCP9808_temperature = m_tempSensor.readAmbient();  // for the MCP9808
				...
				{
					if (m_DS3231_temperature & 0x0200)
						m_DS3231_temperature |= 0xFE00; // sign extend negative number
...
void OnboardDisplay::Update(int8_t updmode)
{
	...
	switch(curstate)
	{
		case EVSE_STATE_A: // not connected
			...
			#endif // LCD16X2 Adafruit RGB LCD
			// n.b. blue LED is off
			break;
	...
	if (((curms-m_LastUpdateMs) >= 1000) || (updmode == OBD_UPD_FORCE))
	{
		...
		if (!g_EvseController.InHardFault() && ((curstate == EVSE_STATE_GFCI_FAULT) || (curstate == EVSE_STATE_NO_GROUND)))
		{
			#ifdef LCD16X2
